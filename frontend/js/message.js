...
function 启动消息催促() {
  ...
  setInterval(() => {
    ...
    // 高概率催促（80%概率）
    if (Math.random() < 0.8 && 未完成.length > 0) {
      const 角色 = ['小陈', '赵经理', '小周', '于总'];
    ...
    if (Math.random() < 0.4) {
      const 消息池 = [
        ...
        { 角色: '小陈', 类型: '提醒', 内容: '郭哥！午饭时间到了！别忘记吃饭！🍱' },
        { 角色: '赵经理', 类型: '催促', 内容: '郭哥，今天的任务进度我看了一眼...不太行啊。' },
        { 角色: '小陈', 类型: '提醒', 内容: '郭哥！别忘了回复消息哦~' },
        { 角色: '李老师', 类型: '提醒', 内容: '郭哥，下班时间快到了，注意效率~' },
        { 角色: '于总', 类型: '通知', 内容: '这个月的数据报告，明天之前交到我桌上。' },
        { 角色: '小周', 类型: '聊天', 内容: '郭哥...素材我还在等...不着急的...大概...' },
        { 角色: '赵经理', 类型: '催促', 内容: '竞争对手又在涨粉了，你还在等什么？' },
      ];
  ...
  setTimeout(() => {
    ...
    const 未完成 = 今日.filter(t => !t.completed);
    if (未完成.length > 0) {
      添加新消息('小陈', '提醒', `郭哥！你今天还有${未完成.length}个任务没完成呢！加油鸭~💪`);
    } else {
      添加新消息('小陈', '提醒', '郭哥早呀！今天也要元气满满哦~ ☀️');
    }
  }, 2 * 60 * 1000);
}
